...
if [[ -f ${gwldir}/v${band}m.txt && -s ${gwldir}/v${band}m.txt ]]
then
...
CONNFAILREASON=`egrep -i "nable to establish connection to remote|Exchange failed|i/o timeout|Missing argument," $LOGFILE  | awk -F":" '{print $4}' | tail -1`
CONNFAILED=`egrep -i "nable to establish connection to remote|Exchange failed|i/o timeout|Missing argument," $LOGFILE  | wc -l`
...
then
	cat ${gwldir}/good-gws.txt ${gwldir}/good-gws-logs.txt | sort | uniq > ${gwldir}/good-gws-combined.txt
        for gws in `cat ${gwldir}/good-gws-combined.txt`
                do grep $gws ${gwldir}/v${band}m.txt >> ${gwldir}/gg-${band}m.txt
        done
...
then
	...
	shuf $gwldir/${band}m-filtered.txt | sed 's/VARA 500/VARA_500/g' | sed 's/VARA 2750/VARA_2750/g' > ${station_list_filtered}  
